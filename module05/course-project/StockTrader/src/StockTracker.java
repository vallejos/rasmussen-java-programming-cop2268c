...
class StockTracker extends JFrame implements ActionListener, WindowListener, Activator, STAction
{
    ...
    JButton jbtAddStock, jbtDelStock, jbtLogout, jbtViewStock, jbtListStocks;
    JButton jbtAddUser, jbtDelUser, jbtUpdUser, jbtListUsers, jbtListStocksUser;
            ...
            throws ClassNotFoundException,SQLException
    {
        ...
        // components for stocks panel
        JPanel p7= new JPanel();
        p7.setLayout(new GridLayout(2,2,5,5)); // rows, cols, hgap, vgap
        jbtAddStock = new JButton("Add Stock");
        jtfStockSymbol = new JTextField(4);
        jbtViewStock = new JButton("View Stock Details");
        jbtListStocks = new JButton("List Available Stocks");
        p7.add(jbtAddStock);
        p7.add(jtfStockSymbol);
        p7.add(jbtViewStock);
        p7.add(jbtListStocks);
        p7.setBorder(new TitledBorder("Manage Stocks"));

        // components for stock holdings panel
        JPanel p2= new JPanel();
        p2.setLayout(new GridLayout(1,2,5,5)); // rows, cols, hgap, vgap
        jbtDelStock = new JButton("Delete Stock Holding");
        jcbStockList = new JComboBox();
        buildStockList();
        p2.add(jbtDelStock);
        p2.add(jcbStockList);
        p2.setBorder(new TitledBorder("Manage Stock Holdings"));

        // this panel will hold p7 and p2
        ...
        jbtListUsers = new JButton("List All Users");
        jbtListStocksUser = new JButton("List Stocks for User");
        
        if(user.isAdmin())
        {
            JPanel p3= new JPanel();
            p3.setLayout(new GridLayout(3,2,5,5)); // rows, cols, hgap, vgap
            p3.add(jbtAddUser);
            p3.add(jbtUpdUser);
            p3.add(jbtDelUser);
            p3.add(jbtListUsers);
            p3.add(jbtListStocksUser);
            p3.setBorder(new TitledBorder("Manage Users"));
        ...
        jbtViewStock.addActionListener(this);
        jbtListStocks.addActionListener(this);
        ...
        jbtListStocksUser.addActionListener(this);
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        else if(e.getSource() == jbtViewStock)
        {
            String stockSymbol = JOptionPane.showInputDialog(this,
                    "Please input the a Stock Symbol to view its Description",
                    "Enter Stock Symbol",
            ...
            if(stockSymbol != null)
            {
                try
                {
                    String stockDesk = db.getStockDesc(stockSymbol);
                    if(stockDesk != null)
                    {
                        // show stock desc
                        JOptionPane.showMessageDialog(this,
                                stockSymbol + ": " + stockDesk,
                                "Stock Description",
                                INFORMATION_MESSAGE);
                    }
                    else
        ...
        else if(e.getSource() == jbtListStocks)
        {
            try
            {
                UserMaintFrame um = new UserMaintFrame(null, LISTSTOCKS, db, this); // do list
                setVisible(false);
                um.setVisible(true);
            }
            catch(ClassNotFoundException ex){}
        ...
        else if(e.getSource() == jbtListStocksUser)
        {
            String userID = JOptionPane.showInputDialog(this,
                    "Please input ID of user to view the Stocks",
                    "Enter User ID",
                    QUESTION_MESSAGE);			// updated for v5.0
            if(userID != null)
            {
                try
                {
                    User stockUser = db.getUser(userID);
                    if(stockUser != null)
                    {
                        // show 
                        UserMaintFrame um = new UserMaintFrame(stockUser, LISTSTOCKSUSER, db, this); // do list
                        setVisible(false);
                        um.setVisible(true);
                    }
                    else
